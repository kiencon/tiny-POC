...
import { COLOR_PICKER_INSIDE, NAME_SECTION } from '../../../config/constant';
...
const ColorSection = ({ informationCompany, dispatch }) => {
  ...
  useEffect(() => {
    const handleClickOutsideColorPicker = e => {
      const insideColorPicker = COLOR_PICKER_INSIDE.find(className => (
        e.target.className.toString().includes(className)
      ));
      const conditionalsCheckInside = !!insideColorPicker || !e.target.className;

      if (ref.current && !ref.current.contains(e.target)) {
        if (!conditionalsCheckInside) {
      ...
      } else if (colors.length && isAdd && colorPick) {
        const newColors = [colorPick, ...colors];
        setColors(newColors);
        setAdd(false);
        setColorPickerIndex('');
